...
def count_images(args):
    if os.path.isdir(args.datapath):
        ...
        if not args.test:
            ...
            print('--- Train: {:>6} imgs, together with {:>6} masks'.format(len(glob.glob(os.path.join(x_train_dir, '*.png'))),
                                                                                   len(glob.glob(os.path.join(y_train_dir, '*.png')))))
            print('     Test: {:>6} imgs, together with {:>6} masks'.format(len(glob.glob(os.path.join(x_test_dir, '*.png'))),
