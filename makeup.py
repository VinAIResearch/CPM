...
#         self.face = face/255
    ...
    def get_texture(self):
        texture = cv2.remap(self.face, self.pos[:,:,:2].astype(np.float32), None, interpolation=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=(0))
        return texture
    
    def get_seg(self):
        texture = cv2.remap(self.face_seg, self.pos[:,:,:2].astype(np.float32), None, interpolation=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=(0))
        return texture
...
#         mask_out_eye = cv2.dilate(self.mask_out_eye, np.ones((10, 10),np.uint8), iterations = 1)
    ...
    def location_to_crop(self, mini = False):
        if mini:
            ...
            x2, y2, x1, y1 = np.min(idx[1]), np.max(idx[1]), np.min(idx[0]), np.max(idx[0])
