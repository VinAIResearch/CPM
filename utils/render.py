...
def render_texture(vertices, colors, triangles, h, w, c=3):
    ...
    tri_depth = (vertices[2, triangles[0, :]] + vertices[2, triangles[1, :]] + vertices[2, triangles[2, :]]) / 3.0
    tri_tex = (colors[:, triangles[0, :]] + colors[:, triangles[1, :]] + colors[:, triangles[2, :]]) / 3.0
...
def map_texture(
    ...
    for y in range(h):
        for x in range(w):
            ...
            # src
            src_texel = (
                w0 * src_vertices[:2, tri[0]] + w1 * src_vertices[:2, tri[1]] + w2 * src_vertices[:2, tri[2]]
            )  #
            #
...
def get_triangle_buffer(vertices, triangles, h, w):
    ...
    triangle_buffer = (
        np.zeros_like(depth_buffer, dtype=np.int32) - 1
...
def vis_of_vertices(vertices, triangles, h, w, depth_buffer=None):
    ...
    for i in range(vertices.shape[1]):
        ...
        if (
            np.floor(vertex[0]) < 0
            or np.ceil(vertex[0]) > w - 1
            or np.floor(vertex[1]) < 0
            or np.ceil(vertex[1]) > h - 1
        ):
            continue
...
def prepare_tri_weights(vertices, triangles, h, w):
    """prepare triangle weight map
    Args:
        vertices: 3 x nver
        triangles: 3 x ntri
        h: height
    ...
    dst_triangle_buffer = np.zeros((h, w), dtype="int") - 1
    depth_buffer = np.zeros([h, w]) - 999999.0
    # triangle depth: approximate the depth to the average value of z in each vertex(v0, v1, v2), since the vertices are closed to each other
    tri_depth = (vertices[2, triangles[0, :]] + vertices[2, triangles[1, :]] + vertices[2, triangles[2, :]]) / 3.0
    for i in range(triangles.shape[1]):
        ...
        for u in range(umin, umax + 1):
            for v in range(vmin, vmax + 1):
                if tri_depth[i] > depth_buffer[v, u] and isPointInTri([u, v], vertices[:2, tri]):
                    depth_buffer[v, u] = tri_depth[i]
                    w0, w1, w2 = get_point_weight([u, v], vertices[:2, tri])
                    weights[v, u, :] = [w0, w1, w2]
                    dst_triangle_buffer[v, u] = i
    return weights, dst_triangle_buffer


def render_by_tri(colors, triangles, weights, dst_triangle_buffer, h, w, c=3, mapping_type="weight"):
    """render mesh by pre-computed triangle weight map
    Args:
        colors: 3 x nver
        triangles: 3 x ntri
        weights: h x w x 3
    ...
    for y in list(range(h)):
        for x in list(range(w)):
            ...
            if mapping_type == "nearest":
                ...
                if wi[ind] < wi[2]:
                    ind = 2
                image[y, x, :] = colors[ind, :]
            # bilinear
            elif mapping_type == "weight":
                image[y, x, :] = wi[0] * colors[:, tri[0]] + wi[1] * colors[:, tri[1]] + wi[2] * colors[:, tri[2]]
    return image
